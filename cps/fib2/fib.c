...
#else // remove boilerplate with a macro

// Uses statement-expressions, a compiler extension. This isn't standard
// compliant, but we need to save the closure, cl, so we can return it,
// and we cannot do that using only the comma operator. If you permit the
// macro to declare variables, you can easily get around it, though. Just let
// it declare the variable cl and you can return it in the functions below.
// If you use this macro, compiler extension or not, you don't strictly need
// to define the new_<type>_closure functions; the macro will do the same work
// for you.

// clang-format off
#define alloc_new_frame(TYPE, ...)                                    \
    ({                                                                \
        closure cl = vs_stack_alloc_frame_type(&closure_stack, TYPE); \
        *get_closure_frame(TYPE, cl) = (TYPE){__VA_ARGS__};           \
        cl;                                                           \
     })
// clang-format on
...
static inline closure new_ret_closure(void)
{
    return alloc_new_frame(struct ret_frame, .fn = ret);
}

static inline closure new_fib2_closure(int f1, closure k)
{
    return alloc_new_frame(struct fib2_frame, .fn = fib2, .f1 = f1, .k = k);
}

static inline closure new_fib1_closure(int n, closure k)
{
    return alloc_new_frame(struct fib1_frame, .fn = fib1, .n = n, .k = k);
}
